...
def cubic_bezier(P0, P1, P2, P3, t):
    ...
    slope = np.divide(dy_bezier, dx_bezier,
                      out=np.full_like(dy_bezier, 1000),
                      where=dx_bezier != 0)
...
def LEI_airfoil(tube_size, c_x, c_y, LE_config, LE_tension, e, TE_angle, TE_cam_tension, TE_tension, LE_fillet):
    ...
    if not flat:
        ...
        D_P21_TE = sqrt((1-P21[0])**2 + P21[1]**2)                              # distance from max camber to TE
        reflex_angle = np.radians(TE_angle) + atan(P21[1]/(1-P21[0]))           # reflex angle
        P22 = np.array([P3[0] - D_P21_TE * TE_tension * cos(reflex_angle),
                        D_P21_TE * TE_tension * sin(reflex_angle)])             # TE top second control point, based on tension para related to the TE distance


    # Flat configuration if cy is below tube radius
    else:
        c_x = 4*radius
        seam_a = np.radians(90 + np.degrees(np.arcsin(radius/(1-radius))))       # Seam angle
        P1 = np.array([radius * (1 - np.cos(seam_a)), radius * np.sin(seam_a)])  # Seam location
        ...
        P11 = np.array([(c_x-P1[0])*0.33+P1[0], slope * ((c_x-P1[0])*0.33+P1[0]) + intercept])
        P12 = np.array([(c_x-P1[0])*0.66+P1[0], slope * ((c_x-P1[0])*0.66+P1[0]) + intercept])
        P2 = np.array([c_x, slope * c_x + intercept])
        P21 = np.array([c_x + TE_cam_tension * (1-c_x), slope * (c_x + TE_cam_tension * (1-c_x)) + intercept])
        D_P21_TE = sqrt((1-P21[0])**2 + P21[1]**2)                                # distance from max camber to TE
        reflex_angle = np.radians(TE_angle) + atan(P21[1]/(1-P21[0]))             # reflex angle
        P22 = np.array([P3[0] - D_P21_TE * TE_tension * cos(reflex_angle),
                        D_P21_TE * TE_tension * sin(reflex_angle)])               # TE top second control point, based on tension para
    ...
    P51 = np.array([c_x + TE_cam_tension * (1-c_x), P21[1]-e])              # TE lower skin first control point, closest to max camber
    P4 = np.array([1 - e * sin(reflex_angle), 0 - e * cos(reflex_angle)])   # TE lower point
    P52 = np.array([P4[0] - D_P21_TE * TE_tension * cos(reflex_angle),
                    D_P21_TE * TE_tension * sin(reflex_angle) + P4[1]])     # TE lower skin 2nd control point
    ...
    if fillet_automatic:
        ...
        index_lower = np.abs(lower_surface_init[:, 0] - P6[0]*1.18).argmin() # Defines the intersection point P63
    else:
    ...
    if fillet_automatic:
        D_fillet1 = 0.55 * np.linalg.norm(P63 - P6)                          #LE_fillet distance P61 closest to tube
...
def plot_airfoil(fig_file_path, profile_name, LE_tube_points, P1, P11, P12, LE_points, TE_points, P2, P21, P22, P3, round_TE_points, P4, P5, P51, P52, TE_lower_points, P6, P61, P62, P63, fillet_points, seam_a):
    ...
    plt.yticks(np.arange(np.ceil(1.5 * min_y / 0.05) * 0.05, np.ceil(1.2 * max_y / 0.05) * 0.05, 0.05))
    plt.axis('equal')
    plt.xlim([0, 1.05])
