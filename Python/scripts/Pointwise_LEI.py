...
def mesh_generation_pointiwse(glyph_script_path, mesh_file_path, Re, points):
    # Pointwise mesh parameters
    initStep = LEI_Parametric.wall_height(Re)   # Initial mesh layer height [m]
    print(initStep)
    growthFactor = 1.1                      # Growth factor
    dimTotal = 575                          # Total surfaces nodes

    n_TE = 15                               # TE node dimension
    n_LE = int(dimTotal*0.28)               # LE node dimension
    n_upper = int((dimTotal-n_TE-n_LE)/2)   # upper surface node dimension
    n_lower = n_upper                       # lower surface node dimension
    numLayers = 201                         # Number of mesh layers in normal direction

    # Hyperbolic smoothing parameters
    explSmooth = 8.0                        # Explicit smoothing: Default is 0.5; between 0 and 10
    implSmooth = 2 * explSmooth             # Implicit smoothing: Default is 1.0; between 0 and infinity but double explicit smoothing
    kbSmooth = 5.0                          # Kinsey Barth: Default is 0.0;
    ...
    with open(glyph_script_path, "w") as f:
        ...
        # Point LE at upper front spline
        f.write(f"lappend _TMP(split_params) [$_CN(1) getParameter -closest [pw::Application getXYZ [$_CN(1) closestControlPoint [list 0 0 $_DB({40})]]]]\n") #25
        f.write("set _TMP(PW_1) [$_CN(1) split $_TMP(split_params)]\n")
        f.write("unset _TMP(PW_1)\n")
        f.write("unset _TMP(split_params)\n\n")

        # Point fillet at connection lower spline and fillet
        f.write("set _CN(2) [pw::GridEntity getByName con-1-split-2]\n")
        f.write(f"lappend _TMP(split_params) [$_CN(2) getParameter -closest [pw::Application getXYZ [$_CN(2) closestControlPoint [list 0 0 $_DB({350})]]]]\n") # 370 fillet_loc-5
        ...
        # Create 3D block by translation 1 step in z-direction
        f.write("set _TMP(mode_1) [pw::Application begin Create]\n")
